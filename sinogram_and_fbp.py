...
for i, angle in enumerate(theta):
    rotated_image = rotate(fantom, angle, mode='constant', cval=0, reshape=False)
...
plt.imshow(sinogram_origigi, cmap='gray', origin='lower',aspect='auto')
plt.title('Matrica sa tri kruga')
plt.show()
...
def apply_filter(projection, filter_type):
    ...
    if filter_type == 'ram_lak':
        ...
        # Implementacija Shepp-Logan filtera
        pass
    elif filter_type == 'hann':
        # Implementacija Hann filtera
        pass
...
filter_type = 'ram_lak'  # Možete promeniti tip filtera
